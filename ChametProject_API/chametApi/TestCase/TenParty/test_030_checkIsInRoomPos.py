...
@allure.story('校验用户是否在直播间麦位')
class Test_CheckIsInRoomPos(object):
    ...
    def test_CheckIsInRoomPos(self, caseData):
        time.sleep(5)
        user_type = 'anchor'
        allLogin(user_type)
        # 从文件中读取bizCode值
        ...
        # 断言用户是否在直播间麦位
        assert returnValue['info']['isInRoomPos'] == 1

if __name__ == '__main__':
    pytest.main(['-s', '-v', 'test_030_checkIsInRoomPos.py'])
