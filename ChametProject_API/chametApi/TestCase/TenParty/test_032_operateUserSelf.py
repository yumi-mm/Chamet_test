...
@allure.story('用户对自己的操作（开启麦克风）')
class Test_OperateUserSelf(object):
    datas = readYaml(os.path.realpath(__file__)[:-3] + '.yaml')
    cases = createCaseData(datas,user_type_1 = 'audiencd')
    ...
    def test_OperateUserSelf(self, caseData):
        time.sleep(1)
        user_type = 'audiencd'
        allLogin(user_type)
        # 从文件中读取bizCode值
...
if __name__ == '__main__':
    pytest.main(['-s', '-v', 'test_032_operateUserSelf.py'])
